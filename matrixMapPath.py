import copy
class getMapInfo:
  def __init__(self, prisonLayout):
...
class point:
  def __init__(self, y=0, x=0, prisonLayout):
    try:
      self.mapInfo = getMapInfo(prisonLayout)
      self.coord = [y, x]
...
def solution(prisonLayout):
  def testing(fromPoint):
    mapInfo = getMapInfo(prisonLayout)
    mapInfo.get_doors()
    mapInfo.get_walls()
    ...
    def heaven_is_in_the_other_side_of_the_wall():
      for potentialBreakPoint in mapInfo.pointsOfWalls:
        the_wall_is_made_in_china = breakPoint(potentialBreakPoint[0], potentialBreakPoint[1], prisonLayout)
