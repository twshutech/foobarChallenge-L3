def solution(prisonLayout):
  ...
  class getMapInfo:
    ...
    def inspect_path(self, fromPoint):
      ...
      # print(self.horizontal_search(self.floorMap[fromPoint[0]], 0))
      # print(self.vertical_search(self.flipFloorMap[fromPoint[1]], 0))
    ...
    def get_path_to_the_heaven():
      def scanHV(turning_point):
        ...
        if mapInfo.linearCheck(index_of_column, turning_point[1])==True:
          return
          #print('linear x',[[turning_point[0],j] for j in index_of_column])
        if mapInfo.linearCheck(index_of_row, turning_point[0])==True:
          return
          #print('linear y',[[i, turning_point[1]] for i in index_of_row])

      for try_this_wall in potentialBreakPoints:
        # mapInfo.face_to(try_this_wall)
        ...
        # print('path from breakpoint to endPoint',reversed_path)
        # print('length of breakpoint to endpoint',len(mapInfo.path_to_breakpoint(try_this_wall)))
        # print('path from entree_and_sortie:',mapInfo.path_to_breakpoint(try_this_wall))
